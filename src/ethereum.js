...
export default class Ethereum {
    constructor(
        abi,
        contractAddress,
        readProviderUrl = 'http://127.0.0.1:7545'
    ) {
        ...
        this.readWeb3 = new Web3(
            new Web3.providers.HttpProvider(readProviderUrl)
        )
    ...
    async getEventsForBlock(blockNumber, numRetries = 0) {
        ...
        try {
            ...
            if (numRetries < 100) {
                logger.log(
                    'warn',
                    `Warning, error happened while contacting RPC, retrying (attempt ${numRetries})`
                )
                await sleep(1200)
    ...
    async readAllEvents(fromBlock, toBlock, { skipBlocks, batchSize }, fn) {
        ...
        for (const batch of batches) {
            const promises = batch.map((job) =>
                this.getEventsForBlock(job.blockNumber)
            )
